...
import {
    Calculator, X, Package, Calendar, Clock, Square, Tag, Wallet, Check, Lightbulb,
} from "lucide-react";
            ...
            >
                ...
                <div className="p-5 space-y-5">
                    ...
                    <div>
                        ...
                        <p className="flex items-start gap-1.5 text-[11px] text-muted-foreground mt-2">
                            ...
                            <span>Contoh: isi <b>2 unit</b> + <b>3 hari</b> → tampil di Qty sebagai <b>&quot;2 unit x 3 hari&quot;</b> (terpisah, bukan digabung 6 unit-hari). Harga satuan di-scale otomatis.</span>
                        </p>
                    </div>
                    ...
                    <div className="rounded-xl border-2 border-primary/20 bg-primary/5 p-4">
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                <div className="text-xs text-muted-foreground">Qty (tampil di tabel)</div>
                                <div className="text-xl font-bold nums text-foreground leading-tight">
                                    {result.quantity || 0}{" "}
                                    <span className="text-sm font-normal text-muted-foreground">{result.unit}</span>
                                </div>
                                <div className="text-[10px] text-muted-foreground mt-0.5">
                                    Harga satuan: <b className="nums">{fmtRp(result.pricePerUnit)}</b>
                                </div>
                            </div>
                            <div>
                        ...
                        <div className="pt-3 border-t border-dashed border-border">
                            ...
                            <div className="italic text-foreground text-sm break-words">
                                {result.descriptionText || (
                                    <span className="text-muted-foreground/60 not-italic">(isi nama item & faktor)</span>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <div className="sticky bottom-0 bg-card border-t border-border px-5 py-4 flex gap-3 rounded-b-2xl">
                    <button
                        type="button"
                        onClick={onCancel}
                        className="flex-1 px-4 py-3 border-2 border-border rounded-lg text-base font-semibold text-foreground hover:bg-muted cursor-pointer transition-colors"
                    >
                        Batal
                    </button>
                    <button
                        ...
                        className="flex-[2] inline-flex items-center justify-center gap-2 px-4 py-3 rounded-lg text-base font-bold text-white bg-primary hover:bg-primary/90 disabled:bg-muted disabled:text-muted-foreground disabled:cursor-not-allowed shadow-sm transition-colors"
                    >
...
const colorMap: Record<string, { ring: string; iconBg: string; iconText: string; focusBorder: string }> = {
    blue: { ring: "border-primary/30", iconBg: "bg-primary/15", iconText: "text-primary", focusBorder: "focus-within:border-primary" },
    emerald: { ring: "border-success/30", iconBg: "bg-success/15", iconText: "text-success", focusBorder: "focus-within:border-success" },
    violet: { ring: "border-info/30", iconBg: "bg-info/15", iconText: "text-info", focusBorder: "focus-within:border-info" },
    amber: { ring: "border-warning/30", iconBg: "bg-warning/15", iconText: "text-warning", focusBorder: "focus-within:border-warning" },
};
...
function BigNumField({
    ...
    return (
        <div className={`rounded-lg border-2 ${c.ring} ${c.focusBorder} bg-card p-2.5 transition-colors`}>
            ...
            <input
                ...
                min="0"
                inputMode="numeric"
                className="w-full bg-transparent outline-none text-2xl font-bold nums text-right text-foreground placeholder:text-muted-foreground/30"
            />
        </div>
    );
}
