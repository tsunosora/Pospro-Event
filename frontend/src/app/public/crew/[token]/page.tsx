...
import { CalendarDays, MapPin, User, Loader2, Camera, CheckCircle2, Clock, AlertCircle, Crown, Paperclip } from "lucide-react";
    ...
    return (
        <div className="min-h-screen bg-muted/20 py-6 px-4">
            <div className="max-w-md mx-auto space-y-4">
                {/* Header */}
                <div className="bg-primary text-primary-foreground rounded-xl p-4 shadow">
                        ...
                        <div className="mt-2 pt-2 border-t border-border flex items-center gap-2 text-xs">
                            ...
                            {data.team.leader && (
                                <span className="text-muted-foreground">
                                    · <Crown className="h-3 w-3 inline mr-0.5" />{data.team.leader.name}
                ...
                {/* Check-in */}
                <div className={`bg-card border rounded-xl p-4 ${data.startedAt ? "border-success/30" : "border-warning/30"}`}>
                    <div className="flex items-center gap-2 mb-2">
                        <Clock className={`h-5 w-5 ${data.startedAt ? "text-success" : "text-warning"}`} />
                        <h2 className="font-semibold">Check-in (Mulai Tugas)</h2>
                        {data.startedAt && <CheckCircle2 className="h-4 w-4 text-success ml-auto" />}
                    </div>
                ...
                {/* Check-out */}
                {status !== "ASSIGNED" && (
                    <div className={`bg-card border rounded-xl p-4 ${data.finishedAt ? "border-success/40" : "border-info/30"}`}>
                        <div className="flex items-center gap-2 mb-2">
                            <CheckCircle2 className={`h-5 w-5 ${data.finishedAt ? "text-success" : "text-info"}`} />
                            <h2 className="font-semibold">Check-out (Selesai)</h2>
                            {data.finishedAt && <CheckCircle2 className="h-4 w-4 text-success ml-auto" />}
                        </div>
                        ...
                        {data.finishedAt ? (
                            <div className="text-sm space-y-1">
                                ...
                                <div className="bg-success/15 text-success rounded-lg p-2 text-xs mt-2 text-center font-medium flex items-center justify-center gap-1">
                                    <CheckCircle2 className="h-3.5 w-3.5" /> Tugas selesai. Terima kasih!
                                </div>
                            </div>
                        ) : (
                            <CheckForm
    ...
    return (
        <div className="space-y-3">
            <div>
                ...
                />
                {photo && <div className="text-[10px] text-muted-foreground mt-1 flex items-center gap-1"><Paperclip className="h-3 w-3" />{photo.name}</div>}
            </div>
            <div>
            ...
            {error != null && (
                <div className="bg-destructive/12 text-destructive text-xs rounded-lg p-2">
