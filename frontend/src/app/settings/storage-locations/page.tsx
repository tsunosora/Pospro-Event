        ...
        <div className="space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button
                    ...
                    className="inline-flex items-center gap-1.5 bg-primary text-primary-foreground px-3 py-1.5 rounded text-sm hover:opacity-90 transition-opacity cursor-pointer"
                >
            ...
            {isLoading ? (
                ...
                <div className="space-y-4">
                    {grouped.filter((g) => warehouseFilter === "ALL" || g.warehouse.id === warehouseFilter).map((g) => (
                        <div key={g.warehouse.id} className="border border-border rounded-xl overflow-hidden">
                            ...
                            {g.items.length === 0 ? (
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody>
                                        {g.items.map((l) => (
                                            <tr key={l.id} className="border-t border-border hover:bg-muted/10 transition-colors">
                                                <td className="p-2 font-mono text-xs">{l.code}</td>
                                                <td className="p-2">{l.name}</td>
                                                <td className="p-2 text-xs text-muted-foreground">{l.notes ?? "—"}</td>
                                                <td className="p-2 text-center">
                                                    <span className={`text-xs px-1.5 py-0.5 rounded ${l.isActive ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}>
                                                ...
                                                <td className="p-2 text-right">
                                                    <button
                                                        ...
                                                        className="text-xs hover:underline mr-2 cursor-pointer transition-colors"
                                                    >Edit</button>
                                                    <button
    ...
    return (
        ...
        >
            <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={onCancel} className="ml-auto text-xs text-muted-foreground hover:underline inline-flex items-center gap-1 cursor-pointer transition-colors">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                <div>
                    ...
                    <select
                        ...
                        className="w-full border border-border bg-card rounded px-2 py-1.5 text-sm mt-0.5"
                    >
                ...
                <div>
                    ...
                    <input
                        ...
                        className="w-full border border-border bg-card rounded px-2 py-1.5 text-sm mt-0.5 font-mono"
                        placeholder="A-3"
                    />
                </div>
                <div>
            ...
            <div>
                <button
                    type="submit"
                    disabled={pending}
                    className="inline-flex items-center gap-1.5 bg-primary text-primary-foreground px-3 py-1.5 rounded text-sm hover:opacity-90 transition-opacity disabled:opacity-50 cursor-pointer"
                >
